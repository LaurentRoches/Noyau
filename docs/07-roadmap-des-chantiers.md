...
**Révision :** 3.4 — 22 septembre 2026.
...
8. `feat(domain): extract CanonicalJson and carry combat-start gold` — les règles d'octets de D-19 quittent `CombatLogSerializer` pour `CanonicalJson`, que le snapshot partage ; `goldAtCombatStart` devient un paramètre **requis** de `CombatBoard` et de `CombatBoardFactory::createBoard()` (D-16). **Fait le 21/09/2026.**
9. `feat(domain): add the board photograph and canonical side assignment` — `BoardSnapshot` dans `App\Domain\Snapshot`, `CombatVestige::getDefinition()`, et l'attribution canonique retirée du commit 5 : A devient le plateau à la plus petite photographie, l'ordre des arguments tranchant à égalité (D-19, `04` §3.6). Les deux ne sont pas séparables — l'attribution compare des photographies, et un test d'attribution sans photographie n'affirme rien. **Aucune ligne de frontend**, comme annoncé. **Fait le 21/09/2026.**
10. `feat(domain): add the board record envelope and its two versions` — `BoardRecord` et `SnapshotRecipe`, `EngineVersion::CURRENT`, `BoardRecord::FORMAT_VERSION`, `contentVersion` fourni par l'appelant (D-16, D-18). **Fait le 21/09/2026.**
...
12. `feat(persistence): pin a computed contentVersion on run records` — empreinte SHA-256 des quatre catalogues canonicalisés, `scripted_opponent.json` compris ; une run dont la version ne correspond plus est rejetée (D-18 volets 2 et 3).
