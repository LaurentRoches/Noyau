...
**Révision :** 3.3 — 21 septembre 2026.
...
8. `feat(domain): add snapshot format, engineVersion and contentVersion` — photographie des `Item` décorés dans l'ordre du plateau, définition du Vestige, héros et compétences, recette et `contentVersion` en provenance, `goldAtCombatStart` embarqué sans condition, version de format distincte d'`engineVersion` (D-16, D-18). **Porte aussi l'attribution canonique des côtés**, retirée du commit 5 : A devient le plateau au snapshot canonique le plus petit, égalité départagée par l'identifiant de combat (D-19, `04` §3.6). Les deux ne sont pas séparables — l'attribution compare des snapshots, et un test d'attribution sans snapshot n'affirme rien. Ne change que `SimulationContext::getSide()` côté moteur, et **aucune ligne de frontend**.
