...
| **Modèle de persistance des statuts (D-20)** | **13/09/2026 — instances indépendantes.** Chaque application crée une instance portant ses stacks, ses ticks et sa source ; aucune fusion. Le régime permanent se stabilise seul entre `floor` et `ceil(durée / cooldown)` par source, sans qu'aucun plafond soit écrit — `ceil` étant le pic juste après une application et la moyenne valant `durée / cooldown` (précision mesurée le 14/09/2026). Écarte le monolithe actuel (discontinuité dès `cd < durée`) et le pool à décroissance (discontinuité déplacée sur `stacks > cd`, plus un rééchelonnage ×7 à ×12 des huit objets à statut). **Périme D-13.** `02` §7.4, `04` §3.3 |
...
| **E-02** | ~~**Statuts fusionnés par type, stacks non bornés**~~ — **résorbée le 14/09/2026** | `ActiveStatus::mergeWith()` fait `stacks +=` et `remainingTicks = max(...)`. `CombatVestige::applyStatus()` indexant sur le seul type, une réapplication avant expiration remet le compteur à plein et empile sans limite. **Le critère est `cooldownTicks < durationTicks`, et quatre des huit objets à statut le franchissent** : `nightfang` (10/30), `shadow_armor` (18/30), `venomous_vial` (20/30), `shadow_venomous_vial` (20/30). `firesteel` et `molotov_cocktail` ne sont bornés qu'à un tick près, et par l'ordre des phases de `Simulator::run()` — vérifié sur le code le 13/09/2026. **L'impact mesuré reste concentré sur un objet** : sur les offensifs l'emballement ne coûte que 6 ticks, `REGEN` bute sur `baseHp`, seul le `WARD` n'est saturé par rien, le bouclier étant sans plafond **par conception**. `shadow_armor` produit **7 155** de bouclier sur 500 ticks sous le modèle à fusion, contre **1 253** sous le modèle par instances — dont 459 de `GAIN_SHIELD` direct dans les deux cas, donc pas du `WARD`. **Mesuré le 14/09/2026** par `SimulatorTest::testShadowArmorProducesABoundedReferenceShieldOverFiveHundredTicks`, qui consigne 1 253 comme valeur de référence. Les estimations antérieures de 1 415 et 1 405 décrivaient le **plafond à 2 stacks de D-13, périmée** : sous un plafond les stacks atteints ne redescendent jamais, alors que les instances expirent et que la moyenne réelle vaut `durée / cooldown` = 1,67 stack par tick. D-20 est donc plus conservateur de 11 % que la solution qu'il remplace. **Second défaut, distinct** : deux objets appliquant le même statut fusionnent, le plus long écrasant la durée du plus court et détruisant l'intention de design des deux. **Tranché par D-20** — modèle par instances indépendantes. **Implémenté** : `CombatVestige` porte une liste d'instances par type, `mergeWith()` a disparu, et `AggregatedStatus` porte la projection exposée aux `CombatEvent` | Chantier 3b, points 1 et 2 — fait |
...
| **E-04** | **`SUNDERING` pénalise deux objets** | `applySundering()` applique le bonus de dégâts *puis* `withCooldownTicks(×1.10)` sans vérifier qu'un bonus a été appliqué. Sur `scutum` et `shadow_scutum`, deux mains sans `DEAL_DAMAGE`, le bonus ne trouve rien mais la pénalité s'applique : **+10 % de cooldown pour zéro bénéfice**. **Chiffré le 14/09/2026** : les deux objets sont à `cooldownTicks: 50`, et `floor(50 × 1,10) = 55`, donc le `floor` n'absorbe pas la pénalité. Une activation perdue sur 500 ticks, soit 50 de bouclier pour `scutum` et 75 pour `shadow_scutum`. **Corrigé le 14/09/2026** : la compétence ne s'applique plus du tout à un deux mains sans `DEAL_DAMAGE` | Chantier 3b, point 4 — fait |
...
| **E-07** | **Champs et méthodes morts** — **deux tiers résorbés le 14/09/2026** | Trois, pas un. ~~`Effect::intervalTicks` est sérialisé vers le frontend et n'est renseigné par aucun des 30 objets~~ : retiré, cinq écritures et zéro lecture, `EffectDTO` perd le champ. ~~`EventDispatcher::dispatch()` et `getListenersFor()` n'ont aucun appelant en production~~ : retirés, `register()` passe en privé, la classe tombe à deux méthodes publiques qui sont exactement les deux appelées en production. **Reste `Trigger`**, non lu nulle part puisque `dispatchForItem()` balaie tous les listeners en ignorant les clés : retirer l'enum amputerait une intention de design, c'est une décision et non un nettoyage | `intervalTicks`, `dispatch()`, `getListenersFor()` : chantier 3b, point 5 — fait. `Trigger` : chantier 3 |
...
1. **Tests de caractérisation.** Ils affirment le comportement **actuel**, y compris ce qui est faux. Ils seront réécrits quand la règle qu'ils caractérisent changera, pas tous au même chantier : ceux du départage des morts simultanées au chantier 2 (D-14), ceux de l'accumulation de stacks au chantier 3b — **fait le 14/09/2026** pour `venomous_vial`, `firesteel`, `molotov_cocktail` et `shadow_armor`.
...
- Un test par objet à statut vérifiant que le nombre d'instances vivantes est encadré par `floor(durationTicks / cooldownTicks)` et `ceil(durationTicks / cooldownTicks)`, la borne haute étant atteinte immédiatement après une application. **Le compte n'est constant que si le cooldown divise la durée** : sur les huit objets à statut, seuls `nightfang` (10/30, constant à 3), `firesteel` et `molotov_cocktail` (20/20, constants à 1) sont dans ce cas. `silent_death` (30/20) et `panacee` (40/30) ont une durée plus courte que leur cooldown et oscillent entre 0 et 1, leur statut s'éteignant entre deux activations : comportement voulu, à ne pas confondre avec une régression. **Formulation corrigée le 14/09/2026** : `ceil` est le pic, pas le régime permanent.
...
**Branche.** `feature/status-instances`, fusionnée le 14/09/2026. *(Renommée le 13/09/2026 — `fix/status-stack-cap` décrivait un plafond qui n'est plus la solution retenue, et `fix/` ne convient pas à un changement de modèle.)*
