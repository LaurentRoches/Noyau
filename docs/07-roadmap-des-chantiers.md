...
**Révision 3.5 — ce qui change.** Une correction de gouvernance et deux états d'anomalie. **Les marqueurs « Fait le 21/09/2026 » ajoutés par la révision 3.4 sur trois commits du chantier 2 sont retirés** : la révision 3.3 posait, deux paragraphes plus haut, qu'aucun marqueur d'avancement n'entre dans cette liste, et la 3.4 l'a fait quand même. La règle est la bonne — l'en-tête de ce document interdit la mise à jour par session, et l'avancement appartient au résumé de session, classé rang 2 de la hiérarchie d'autorité. **E-05 est résorbée** par l'épinglage de la version de contenu ; sa ligne le dit, conformément à la règle propre au registre d'anomalies. Et la description du commit 12 gagne le code HTTP du rejet, tranché en `04` §7 — c'est du contrat, pas de l'avancement.

**Révision 3.4 — ce qui change.** Trois commits du chantier 2 ont été écrits, et le commit 8 annoncé a **éclaté en quatre** (§5, chantier 2). Une anomalie s'ouvre, **E-14** : la clause de départage de `04` §3.6 était inapplicable, ce qui n'a été vu qu'en l'implémentant — elle avait survécu à deux révisions du document qui la porte. Sans portée en PvE, réelle en PvP.

**Révision 3.3 — ce qui change.** Le **chantier 0 est terminé** — cette fois vérifié, après que la révision 3.1 ait refusé de le rayer sur une affirmation. **E-03 est entièrement résorbée**, et par une non-action : la garde de vie que son reliquat réclamait dans `StatusProcessor` n'a pas été ajoutée et ne le sera pas, l'absence de garde étant la résolution simultanée que D-14 demande. Ce qui manquait était un départage, pas une garde.

**Révision 3.2 — ce qui change.** Le commit 5 a été écrit, et **la première lecture du frontend a défait deux affirmations du corpus**. `04` §8 nommait `combatPlayback.ts` comme le fichier touché par D-19 : il ne contient aucune occurrence de côté. Et `04` §3.6 posait une attribution canonique qui **dépend du format de snapshot**, donc infaisable au rang où le plan la plaçait. D-19 se coupe en deux : le contrat des libellés neutres et du `viewerSide` au commit 5, la valeur canonique au commit 8, qui l'absorbe. **Le compte reste à onze** — rien n'est ajouté, une moitié est déplacée. Aucune décision n'est modifiée.

> **Ce que cette lecture confirme une troisième fois.** Les deux affirmations tombées avaient la même origine : un fichier nommé dans un document sans avoir été ouvert. C'est la cause des deux erreurs de la révision 1.0 relevées en §10, celle de l'argument de D-16 tombé au cadrage, et celle du faux trou de couverture d'`ActionProcessor` corrigé en 3.1. Quatre fois le même geste.
...
| ~~Frontend, **72** tests Vitest sur 10 fichiers~~ — **lu le 20/09/2026** | ~~Le lecteur de rejeu, touché par le passage aux libellés `A`/`B`~~ **Huit fichiers touchés, 25 tests sur 72** — et jamais `combatPlayback.ts`, que `04` §8 désignait sans l'avoir ouvert. La lecture a aussi montré que D-19 **retire** une information au journal au lieu de la renommer, d'où le champ `viewerSide` |
...
| **E-03** | ~~**Gardes de mort incomplètes**~~ — **entièrement résorbée le 21/09/2026** | ~~`Simulator::run()` n'a **aucun contrôle entre statuts et enrage** : si l'adversaire meurt d'un poison, l'enrage s'exécute quand même, frappe le joueur en premier et peut le tuer — **une victoire devient une défaite**~~ **Corrigé** : la boucle rompt entre les deux phases, et `SimulatorTest::testPlayerWinsWhenPoisonKillsOpponentEvenIfEnrageWouldTriggerSameTick` l'exerce avec un enrage à 1000 de dégâts dès le tick 1. ~~Le commentaire d'`EnrageProcessor` affirme rétablir un double KO « structurellement impossible »~~ **réécrit** : il décrit l'invariant réel et nomme le trou restant. ~~**Reste** : `StatusProcessor::processTick()` n'a **aucune garde de vie** et pulse les deux plateaux~~ — **résorbée le 21/09/2026, et pas en ajoutant la garde.** D-14 fait de la phase de statuts une phase **simultanée** : son absence de garde **est** la règle, et le `break` envisagé l'aurait implémentée à l'envers. Ce qui manquait n'était pas une garde mais un **départage**, désormais en place | Chantier 2, commit 7 — fait |
...
| **E-15** | ~~**L'ordre des effets d'un objet dépend de l'ordre des arguments de `run()`**~~ — **résorbée le 26/09/2026** | *(relevée le 26/09/2026, en cadrant le rejeu octet pour octet)* `EventDispatcher` indexait ses écouteurs par `Trigger`, et `Simulator::run()` enregistre les deux plateaux **dans l'ordre de ses arguments** : les clés de cette table se créaient donc dans l'ordre des triggers rencontrés, et `dispatchForItem()` les parcourant, un objet portant **deux `Trigger` différents** voyait ses effets dépliés dans un ordre qui dépendait de quel plateau s'était enregistré le premier — et de quels triggers portait l'objet de l'autre plateau. **Mesuré : `run($a, $b)` et `run($b, $a)` produisaient deux journaux différents à l'octet près**, sur les mêmes plateaux et la même graine, avec un déroulé réellement divergent. NF-01 ne tenait pas. **Trois documents l'avaient vue sans la voir** : `06` §3 la rangeait depuis la révision 2.1 parmi trois risques d'ordre d'itération, « ouvert, chantier 3 » ; `04` §3.3 la décrivait comme une dette qui « devient réelle au chantier 3 » ; et `04` §3.6 affirmait que l'ordre canonique de `getBoards()` avait refermé la dépendance à l'ordre des arguments — il en avait refermé **une** sur deux. Elle était invisible parce qu'aucun des trente objets du catalogue ne porte deux déclencheurs et que `HeroSkillDecorator` n'en crée pas : le défaut était **latent, pas absent**. Corrigée par une liste plate remplaçant la table indexée ; l'ordre des effets est désormais celui que l'objet porte, donc celui que la photographie archive. `EngineVersion` relevée de 1 à 2 — gratuit tant qu'aucun corpus n'existe, jamais plus après | Chantier 2 — fait |
| **E-14** | **Deux photographies égales ne sont départagées que par l'ordre des arguments** | *(relevée le 21/09/2026, à l'écriture de l'attribution canonique)* `04` §3.6 annonçait un départage « par un identifiant de combat enregistré avec les données d'entrée ». **Cette clause est inapplicable** : l'identifiant de combat est une valeur **unique, partagée par les deux plateaux**, pas une valeur par plateau — aucune fonction de (photoA, photoB, combatId) ne peut ordonner deux photographies égales. §3.6 avait d'ailleurs écarté les identifiants **par plateau** au paragraphe précédent, l'adversaire scripté n'en ayant pas : le remède proposé ne corrigeait pas le défaut qu'il visait, et la contradiction a survécu deux révisions. Le repli est l'ordre des arguments. **Portée réelle : nulle en PvE**, le plateau du joueur et l'adversaire scripté ne photographiant pas à l'identique de façon durable. **Réelle en PvP** : en miroir, le journal est identique dans les deux sens mais désigne « A » comme vainqueur, donc l'ordre décide **quel joueur gagne**. La contrainte qui en découle : l'ordre passé au simulateur devra venir d'une donnée enregistrée avant la simulation — l'enregistrement d'appariement —, jamais d'un rangement local | **Ouvert.** Chantier 11, avec le PvP. `04` §3.6 corrigée en révision 2.5 |
...
|---:|---|---|---|---|
| 1 | ~~**0** — Gardes de mort et caractérisation~~ **terminé le 20/09/2026** | — | S | — |
...
**Le rang 1 est clos.** Les deux puces de caractérisation qui manquaient — la direction de la fureur à travers `Simulator::run()` et sa position dans l'ordre des phases — ont été écrites le 20/09/2026, aucun code de production touché. **Elles ont servi moins de vingt-quatre heures** : le commit 7 du chantier 2 a renversé la première le lendemain, ce qui est précisément ce qu'on attend d'une caractérisation. Sans elle, le renversement aurait été invisible dans le diff.
...
5. `feat(combat): migrate sides to A/B and introduce viewerSide` — `SimulationContext::getSide()` passe de `PLAYER`/`OPPONENT` à `A`/`B`, tous les payloads suivent, et la réponse de `resolveRound` porte le **`viewerSide`** sans lequel le client ne peut plus écrire « ton Vestige ». **L'attribution reste positionnelle** : sa version canonique dépend du format de snapshot et part au commit 8 (voir ci-dessous). Traverse Domaine, Http et frontend — d'où le scope transversal `combat`, un scope `domain` serait faux.
...
8. `feat(domain): extract CanonicalJson and carry combat-start gold` — les règles d'octets de D-19 quittent `CombatLogSerializer` pour `CanonicalJson`, que le snapshot partage ; `goldAtCombatStart` devient un paramètre **requis** de `CombatBoard` et de `CombatBoardFactory::createBoard()` (D-16).
9. `feat(domain): add the board photograph and canonical side assignment` — `BoardSnapshot` dans `App\Domain\Snapshot`, `CombatVestige::getDefinition()`, et l'attribution canonique retirée du commit 5 : A devient le plateau à la plus petite photographie, l'ordre des arguments tranchant à égalité (D-19, `04` §3.6). Les deux ne sont pas séparables — l'attribution compare des photographies, et un test d'attribution sans photographie n'affirme rien. **Aucune ligne de frontend**, comme annoncé.
10. `feat(domain): add the board record envelope and its two versions` — `BoardRecord` et `SnapshotRecipe`, `EngineVersion::CURRENT`, `BoardRecord::FORMAT_VERSION`, `contentVersion` fourni par l'appelant (D-16, D-18).
11. `refactor(domain): rename the context's construction-order accessors` — `getPlayerBoard()`/`getOpponentBoard()` deviennent faux dès le commit 9 et ne sont lus par aucun code de production. Sorti du commit 9 pour la même raison que le commit 8 a été sorti : un renommage sans contenu comportemental rend illisible le diff d'un commit qui déplace le sens de A et de B.
12. `feat(persistence): pin a computed contentVersion on run records` — empreinte SHA-256 des quatre catalogues canonicalisés, `scripted_opponent.json` compris ; une run dont la version ne correspond plus est rejetée (D-18 volets 2 et 3). **Code du rejet, tranché le 23/09/2026 (`04` §7) :** 409 par une exception dédiée `ContentVersionMismatchException`, capturée **avant** la `LogicException` générique, avec un champ `code` facultatif ajouté à `ApiResponse::error()`. Le 409 nu existait déjà : sans le champ, le refus serait indiscernable d'un conflit de séquence, alors que les deux appellent la réaction inverse. **La porte de CI de `04` §10 est explicitement hors de ce commit** — elle n'a aucune valeur de référence à comparer tant que le commit 14 n'existe pas.
13. `feat(application): record combat outcome in RESOLVE_ROUND and apply it on replay` — l'issue est écrite dans la charge utile, le rejeu l'applique au lieu de resimuler, le chemin est réservé à `GameRunReplayer`, `RunController::resolveRound()` ignore tout champ d'issue venu du client (D-18 volet 1, E-11). Enregistrement de combat par manche : snapshots A et B, `combatSeed`, `engineVersion`, `resolution`, `winnerSide`.
14. `test(domain): assert byte-identical replay of a reference combat`.
...
**De onze à quatorze — l'éclatement du commit 8, décidé à l'écriture le 21/09/2026.**

| Mouvement | Commit | Motif |
|---|---|---|
| **Éclaté** | `add snapshot format, engineVersion and contentVersion` | Le commit annoncé contenait **quatre briques indépendantes**. Les garder ensemble aurait produit un message décrivant des choses sans rapport apparent, et un `git bisect` incapable d'en isoler une |
| **Ajouté** | `extract CanonicalJson and carry combat-start gold` | Deux préparatifs qui se tiennent seuls : l'encodeur canonique est un refactor pur, et `goldAtCombatStart` est un champ avec ses propres tests, justifié par D-16 avant même qu'un snapshot existe |
| **Ajouté** | `add the board record envelope and its two versions` | L'enveloppe de provenance ne sert à personne avant le commit 13. Elle est écrite maintenant **parce que le format est irréversible**, pas parce qu'un appelant l'attend |
| **Ajouté** | `rename the context's construction-order accessors` | Un renommage mécanique n'a pas sa place dans un commit qui change le sens de A et de B |

**Ce que l'éclatement a confirmé.** Le commit 9 seul a fait basculer **dix-neuf assertions** de tests moteur — l'adversaire occupe désormais A dans tous les tests où les deux plateaux diffèrent. Noyée dans un commit de quatre briques, cette bascule aurait été indistinguable du reste.

**De quatorze à dix-neuf — l'éclatement des rangs 13 et 14, décidé à l'écriture les 23 et 26/09/2026.**

| Mouvement | Ce qui a été séparé | Motif |
|---|---|---|
| **Éclaté** | Rang 13, `record combat outcome in RESOLVE_ROUND and apply it on replay` | **Trois briques** : l'enum d'issue et le second chemin d'application dans `GameRun` ; l'assemblage de l'enregistrement de combat à chaque manche résolue ; la table, son dépôt et le câblage du contrôleur. La troisième porte une **migration de schéma** et un **ordre d'écriture** — journal d'abord, archive ensuite — qui ne se relisent pas dans le même diff que les deux premières |
| **Éclaté** | Rang 14, `assert byte-identical replay of a reference combat` | Quatre briques aussi, et trois n'étaient pas prévues. Le test de rejeu **ne pouvait pas être écrit** : `CombatVestige` et `CombatHero` réclamaient des entrées de catalogue complètes, donc rejouer une archive aurait exigé d'inventer des champs qu'elle ne porte pas |
| **Ajouté** | `depend on combat profiles instead of catalogue definitions` | Deux ports dans `Domain\Runtime`, implémentés par `Vestige` et `Hero`. Option retenue sur mesure : les trois autres coûtaient la réécriture de **soixante-six sites de construction dans seize fichiers de test**, ou des valeurs fictives. `04` §5.6 |
| **Ajouté** | `hydrate a combat board from its photograph` | L'hydrateur ne reçoit **qu'une chaîne** — pas de tableau décodé, pas de dépôt, pas de chemin de configuration. L'isolement devient structurel plutôt que déclaratif |
| **Ajouté** | `make an item's effect order independent of board order` | E-15, découverte **en cadrant le rejeu** et non en l'écrivant. Un correctif de moteur ne pouvait pas voyager avec un commit de test |
| **Ajouté** | `assert byte-identical replay of a reference combat` | Le rang 14 d'origine, enfin écrivable. Deux tests, une fixture de quatre fichiers et l'outil qui la régénère |

**Ce que l'éclatement du rang 14 a montré.** Le commit de profils est un refactor sans effet observable — 448 tests avant, 448 après —, et c'est exactement ce qui le rend lisible : il ne fait qu'ouvrir un chemin. Le commit de correctif, lui, a rougi deux chaînes figées de `BoardRecordTest` par l'incrément d'`EngineVersion`, ce que le docblock de ce fichier annonçait comme le point de contrôle attendu.

**Critère de sortie — atteint le 26/09/2026.** EX-J0-03 vert. Un snapshot produit, écrit, relu, rejoué, `CombatLog` identique octet pour octet. **Complété en 3.0 :** une run rejouée après un changement de moteur conserve ses issues de manche, et une run dont la `contentVersion` ne correspond plus est rejetée avec un message explicite.

> **Ce que le critère ne couvre pas, et qu'il faut savoir en sortant.** La fixture de référence traverse **six des dix types d'événement**. Un balayage de soixante-dix combats PvE — six graines, douze manches — n'a produit **que des KO**, le plus long à 192 ticks sur 500 : la fureur en exige 450 et le départage l'absence de KO, donc ni l'un ni l'autre n'est atteignable par un combat PvE tel que le jeu est équilibré. Ces quatre chemins restent figés par `SimulatorTest` à l'unité ; ce qui leur manque est le passage par l'archive. **La porte de CI sur l'empreinte de contenu n'est toujours pas écrite** (`04` §10, `06` §4.2) : son second motif d'exclusion est tombé — la valeur de référence existe désormais — mais elle n'a pas été reprise dans ce chantier.
...
**Pourquoi `AURIC`.** Aucune des 18 compétences initiales ne touchait à l'économie, laissant le Doré sans signature de combat. `GAIN_GOLD` demeure écartée : rien n'est gagné en combat, l'or est un **paramètre d'entrée figé au lancement**. *(Corrigé le 21/09/2026 : cette phrase disait « `AURIC` reste un filtre appliqué à l'assemblage du plateau », ce que `02` §2.3.1 contredit depuis sa révision 3.0 — `AURIC` est l'une des deux seules compétences du pool cible qui lisent leur entrée **pendant** le combat. La conclusion tient dans les deux lectures, la prémisse non ; c'est elle qui était périmée. La correction a une conséquence de code : l'or est porté par `CombatBoard`, pas passé au sérialiseur de snapshot.)*
...
**Préalable de mesure ajouté en 3.6, chiffré.** Le premier des deux préalables ci-dessus — remettre à plat l'adversaire scripté — a désormais une mesure. Sur **soixante-dix combats PvE** (six graines × douze manches, catalogue du 26/09/2026), **tous se terminent par un KO**, le plus long à **192 ticks sur 500**. Deux conséquences. La fureur, qui se déclenche au tick 450, **n'est jamais exercée en conditions réelles** — elle n'est donc calibrée par rien, et son unique garde est unitaire. Et cinq runs sur six s'arrêtent à la manche 10, une à la 11 : la durée de run est donc gouvernée par la condition de fin et non par la difficulté. **Calibrer le contenu sans corriger cela reviendrait à équilibrer sur une courbe que personne n'atteint.**
...
**Dix, contre cinq en révision 2.0.** Tout le reste se corrige.
...
| 6 | **L'attribution canonique de A et B** (D-19) | Snapshot canonique le plus petit en octets, **égalité départagée par l'ordre des arguments** *(corrigé en 3.6 — cette ligne portait encore la clause « départagée par l'identifiant de combat enregistré », que E-14 a retirée de `04` §3.6 le 21/09/2026)*. Si l'attribution change, l'initiative de chaque tick change, donc le vainqueur peut changer. **Dépend de la forme du snapshot** : la changer après coup exige de réattribuer tout le corpus |
| 10 | **L'ordre des effets à l'intérieur d'un objet** (E-15) | Il décide de qui frappe avant qui dans un même tick, donc du déroulé entier. Il est désormais celui que l'objet porte, donc celui que la photographie archive — le changer rejouerait tout le corpus différemment. Ce point n'était pas identifié avant le 26/09/2026 : il vivait dans un détail d'implémentation du dispatcher |
